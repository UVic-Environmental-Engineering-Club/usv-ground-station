...
const radiantsToDegrees = (radians: number) =>
  parseInt(((radians * 180) / Math.PI).toFixed(0), 10);

const degreesToRadians = (degress: number) => (degress * Math.PI) / 180;

const getCenterAngleOfRange = (from: number, to: number) => {
  if (from > to) {
...
const getOffsetAngleOfRangeCenter = (from: number, to: number) => {
  if (from > to) {
    return (to + (360 - from)) / 2;
  }
  return (to - from) / 2;
...
const modulus360 = (angle: number) => (angle + 360) % 360;
...
const AngleRange = ({
  ...
  const [isFromAngleHandlerActive, setIsFromAngleHandlerActive] =
    useState(false);
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            transform: `translate(${
              fromAngleHandler.x - offsetHandlerRadius
            }px, ${fromAngleHandler.y - offsetHandlerRadius + 1}px) rotate(${
              fromAngleHandler.angle
            }deg)`,
        ...
        <div
          ...
          style={{
            ...
            transform: `translate(${toAngleHandler.x}px, ${
              toAngleHandler.y - offsetHandlerRadius + 1
            }px) rotate(${toAngleHandler.angle}deg)`,
